...
from src.models.geometry_model import GeomHead
from src.models.model_base import BasicModel
from src.utils.train_utils import EMAModel
from src.utils.math_utils import kl_gaussians
from src.loss.topology_consistency import getTopoLoss
from src.loss.topoloss_latent import get_latent_wasserstein

import sys
sys.path.append("/home/gdemi/multi_view/")
from segmentation.pipeline import unet, vessel_transformer

def calculate_topo_latent_loss(latent1, latent2):
    batch_size = latent1.shape[0]
    channel_size = latent1.shape[1]

    topo_loss = 0.0
    for i in range(batch_size):
        temp_loss = 0.0
        for j in range(channel_size):
            temp_loss += getTopoLoss(latent1[i, j], latent2[i, j])
        temp_loss /= channel_size
        topo_loss += temp_loss
...
def parse_fov_and_eye(text_prompt):
    """Extracts input_fov, target_fov, and eye from text prompt."""
    # Normalize text (lowercase, remove whitespace)
    text = re.sub(r"\s+", "", text_prompt.lower())
    
    # Extract eye (default to left)
    eye = "left"
    if "right" in text:
        eye = "right"
    
    # Extract FOVs
    # fovs = ["od", "nasal", "temporal"]
    fovs = ["nasal", "temporal"]
    # in od-center only condition, we don't give 'input:' info, it's always OD
    # input_fov = next((fov for fov in fovs if f"input:{fov}" in text), None)
    # target_fov = next((fov for fov in fovs if f"target:{fov}" in text), None)
    input_fov = 'OD'#next((fov for fov in fovs if f"input:{fov}" in text), None)
    target_fov = next((fov.capitalize() for fov in fovs if fov in text), None)
    
    if not input_fov or not target_fov:
        raise ValueError(f"Invalid FOV specification: {text_prompt}")
    
    return input_fov, target_fov, eye


def get_gt_se2(text_prompt):  
    positions= []
    for txt in text_prompt:
        # positions.append(get_ground_truth_se2(txt)) # rop FOV changes
        positions.append(get_gt_se2_xray(txt)) # xray FOV changes
    return torch.stack(positions, dim=0)

import numpy as np

def get_gt_se2_xray(text_prompt, default_theta=90, default_translation=0):
    """
    Maps text prompts to SE(2) parameters (θ, dx, dy) for X-ray view synthesis.
    
    Args:
        text_prompt (str): E.g., "lateral to frontal", "frontal to lateral".
        default_theta (float): Default rotation angle (degrees).
        default_translation (float): Default translation magnitude (pixels).
        
    Returns:
        tuple: (θ in radians, dx, dy).
    """
    text_prompt = text_prompt.lower()
    theta, dx, dy = 0.0, 0.0, 0.0
    
    # Define rotation direction and translations based on prompt
    if "given lateral x-ray view, target is frontal x-ray view" in text_prompt:
        # Project 3D rotation (90° around Y-axis) to 2D in-plane rotation (θ = -90°)
        theta = np.deg2rad(-default_theta)
        # Translation depends on patient positioning (empirically set or calibrated)
        dx = default_translation  # Adjust based on your dataset
        
    elif "frontal to lateral" in text_prompt:
        # Inverse rotation (θ = +90°) and translation
        theta = np.deg2rad(default_theta)
        dx = -default_translation  # Opposite direction
        
    else:
        raise ValueError(f"Unsupported prompt: {text_prompt}")
    
    return torch.tensor([theta, dx, dy], dtype=torch.float32)


class DiffusionPipelinePoseEncoder(BasicModel):
    def __init__(self, 
        noise_scheduler,
        noise_estimator,
        ...
        ## x-ray inputs have 2 views: LP and AP, ROP has 3 views: OD center, Nasal and Temporal
        self.geom_head = GeomHead(input_channels=1)
        self.geom_head = self.geom_head.to(self.device) # Move to appropriate device
        self.ssl_geometry_loss_fn = nn.L1Loss()
        self.lambda_ssl = 0.5
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        results = {}
        x_0 = batch['fov_image']
        # condition_label = batch.get('fov_label', None) 
        condition_text = batch.get('fov_text', None)
        condition_img = batch.get('cond_image', None)
        ...
        if self.use_self_conditioning:
            with torch.no_grad():
                if self.image_conditioning:
                    pred, pred_vertical, se2 = noise_estimator(x_t, t, condition_img, condition_text, None) ##
                else:
                    pred, pred_vertical, se2 = noise_estimator(x_t, t, condition_img, condition_text, None) 
        ...
        # ----------------- Reconstruction Loss --------------
        # #### deterministically recover the noise without the sampling step ## Saumya's paper TopoDiffNet
        alpha = self.noise_scheduler.alphas_cumprod[t]
        comp_alpha = 1 - alpha
        pred_x_0 = (x_t - (pred*comp_alpha.sqrt().view(-1, 1, 1, 1))) / alpha.sqrt().view(-1, 1, 1, 1)

        # ## get the real and predicted images
        with torch.no_grad(): real_img = self.latent_embedder.decode(x_0)
        pred_img = self.latent_embedder.decode(pred_x_0)
        loss += self.recon_loss(pred_img, real_img) * 0.1 # reconstruction loss

        # ------------------ Geometry Loss --------------
        translation_predict = self.geom_head(condition_img, pred_img.detach())
        gt_se2 = get_gt_se2(condition_text).to(self.device)
        loss += self.ssl_geometry_loss_fn(translation_predict, gt_se2)
        
        # ----------------- Topology Loss --------------
        if self.global_step > 10000:
        
            # #### !! not to hit OOM, we are using CPU | no grad ####
            ...
            # # ## get the real and predicted images
            # with torch.no_grad():
            #     real_img = self.latent_embedder.decode(x_0) 
            #     pred_img = self.latent_embedder.decode(pred_x_0)

            real_img = (real_img - real_img.min())/(real_img.max()-real_img.min())
            pred_img = (pred_img - pred_img.min())/(pred_img.max()-pred_img.min())

            ## prepare the images for vessel extractor
            real_img = vessel_transformer(real_img)
            pred_img = vessel_transformer(pred_img)

            ## get the vessel mask 
            # with torch.no_grad():
            real_vessel_mask = torch.softmax(unet(real_img), dim=1)[:,1,:,:]
            pred_vessel_mask = torch.softmax(unet(pred_img), dim=1)[:,1,:,:]

            loss += calculate_topo_loss(pred_vessel_mask, real_vessel_mask).to(self.device) * 0.002 # output topology
            # output_topology = calculate_topo_loss(pred_vessel_mask, real_vessel_mask).to(self.device) * 0.01 # output topology

            ## calculate latent topology loss
            latent_fake = (pred_x_0 - pred_x_0.min())/(pred_x_0.max()-pred_x_0.min()) ## normalize the latents
            latent_real = (x_0 - x_0.min())/(x_0.max()-x_0.min())
            loss += calculate_topo_latent_loss(latent_fake, latent_real)# * output_topology  # latent topology
        # ----------------------------- Deep Supervision -------------------------
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            idx = random.randint(0, len(self.condition_test_ds)-1)
            sample = self.condition_test_ds[idx]
            sample_target = sample['fov_image'].unsqueeze(0).to(self.device)
            sample_condition_img = sample['cond_image'].unsqueeze(0).to(self.device)
            # sample_cond = sample['fov_label'].unsqueeze(0).to(self.device) # class label
            sample_cond = sample['fov_text'] # text label

            # sample_cond = condition[0:self.num_samples] if condition is not None else None
            sample_img = self.sample(num_samples=self.num_samples, img_size=x_0.shape[1:], steps=50, cond_img=sample_condition_img, condition=sample_cond).detach()
            ...
            images_final = torch.cat((sample_condition_img*255, sample_target*255, images*255),0)
            save_image(images_final, path_out/f'sample_{log_step}.png', normalize=True)
        
        # torch.cuda.empty_cache() 
        return loss

    
    def forward(self, x_t, t, cond_img=None, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False, un_cond=None):
        ...
        if (condition is not None) and (guidance_scale != 1.0):
            # Model prediction 
            if self.image_conditioning:
                pred_uncond, _, _ = noise_estimator(x_t, t, cond_img=None, condition=un_cond, self_cond=self_cond)
                pred_cond, _, _ = noise_estimator(x_t, t, cond_img=cond_img, condition=condition, self_cond=self_cond)
                pred = pred_uncond + guidance_scale * (pred_cond - pred_uncond)
            else:
                pred_uncond, _, _ = noise_estimator(x_t, t, condition=un_cond, self_cond=self_cond)
                pred_cond, _, _ = noise_estimator(x_t, t, condition=condition, self_cond=self_cond)
                pred = pred_uncond + guidance_scale * (pred_cond - pred_uncond)
            ...
            pred, _, _ =  noise_estimator(x_t, t, cond_img=cond_img, condition=condition, self_cond=self_cond)
            if self.estimate_variance:
    ...
    @torch.no_grad()
    def denoise(self, x_t, steps=None, cond_img=None, condition=None, use_ddim=True, **kwargs):
        self_cond = None 
        ...
        for i, t in enumerate(reversed(timesteps_array)):
            ...
            # UNet prediction 
            x_t, x_0, x_T, self_cond = self(x_t, t.expand(x_t.shape[0]), cond_img, condition, self_cond=self_cond, **kwargs)
    ...
    @torch.no_grad()
    def sample(self, num_samples, img_size, cond_img=None, condition=None, **kwargs):
        template = torch.zeros((num_samples, *img_size), device=self.device)
        x_T = self.noise_scheduler.x_final(template)
        x_0 = self.denoise(x_T, cond_img=cond_img, condition=condition, **kwargs)
        return x_0 
