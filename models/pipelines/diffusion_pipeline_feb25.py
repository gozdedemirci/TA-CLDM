...
loss_wass_fn = WassersteinDistance(q=1)
def get_topo_cc_loss(real_latent, generated_latent):
    '''
        For the generated batched latent representation, compute the topological loss
        with respect to the real batched latent representation.
        Input: real_latent, generated_latent [B, C, H, W]
        Output: topological loss [value]
    '''
    batch_size = real_latent.shape[0]
    channel_size = real_latent.shape[1]
    cpx = CubicalComplex(4)
    cc_real = cpx(real_latent)
    cc_generated = cpx(generated_latent)

    topo_loss = 0
    for i in range(batch_size):
        for j in range(channel_size):
            topo_loss += loss_wass_fn(cc_real[i][j], cc_generated[i][j])
...
class DiffusionPipeline(BasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        output_topology = calculate_topo_loss(pred_vessel_mask, real_vessel_mask).to(self.device) * 0.002 # output topology
    ...
    def forward(self, x_t, t, cond_img=None, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False, un_cond=None):
        ...
        if (condition is not None) and (guidance_scale != 1.0):
            # Model prediction 
            if self.image_conditioning:
                pred_uncond, _ = noise_estimator(x_t, t, cond_img=None, condition=un_cond, self_cond=self_cond)
                pred_cond, _ = noise_estimator(x_t, t, cond_img=cond_img, condition=condition, self_cond=self_cond)
