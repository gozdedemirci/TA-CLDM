...
from src.loss.geometry_affine_loss import GeoLossV0V1
from src.loss.affine_transform import AffineRanges
...
class DiffusionPipelinePoseEncoder(BasicModel):
    def __init__(self, 
        ...
        classifier_free_guidance_dropout=0.15, # Probability to drop condition during training, has only an effect for label-conditioned training 
        num_samples = 1,
        ...
        self.geometry_loss_fn = GeoLossV0V1(img_size_hw=(512, 512), ranges=AffineRanges(max_rot_deg=20, scale_log_range=0.2), use_lpips=True, )
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        condition_text = batch.get('base_text', None)
        # condition_img = batch.get('cond_image', None)

        ### for multi conditioned input
        condition_od_img = batch.get('cond_od_image', None)
        condition_per_img = batch.get('cond_peripheral_image', None)
        ...
        if self.use_self_conditioning:
            with torch.no_grad():
                if self.image_conditioning:
                    # pred, pred_vertical = noise_estimator(x_t, t, condition_img, condition_text, None) ##
                    # pred, pred_vertical, se2 = noise_estimator(x_t, t, condition_img, condition_text, None) ##
                    pred, pred_vertical, se2 = noise_estimator(x_t, t, condition_od_img, condition_per_img, condition_text, None) ##
                else:
                    # pred, pred_vertical = noise_estimator(x_t, t, condition_img, condition_text, None) 
                    pred, pred_vertical, se2 = noise_estimator(x_t, t, condition_od_img, condition_per_img, condition_text, None) 
        ...
        if self.global_step > 20000:
            print("Checking Geo Loss...")
            # loss += F.l1_loss(se2, get_gt_se2(condition_text).to(self.device)) * 0.5
            # loss += self.geometry_loss_fn(se2, condition_img, real_img) * 0.5
            real_img_norm = (real_img - real_img.min())/(real_img.max()-real_img.min())
            loss += self.geometry_loss_fn(se2, condition_od_img, real_img_norm, teacher_partial=True, lambda_v1=0.0) * 0.5

        # ----------------- Topology Loss --------------
        if self.global_step > 40000:
            print("Checking Topology Loss...")
            ...
            ## get the vessel mask 
            # with torch.no_grad():
            with torch.no_grad(): real_vessel_mask = torch.softmax(unet(real_img.detach()), dim=1)[:,1,:,:]
            pred_vessel_mask = torch.softmax(unet(pred_img), dim=1)[:,1,:,:]

            loss += calculate_topo_loss(pred_vessel_mask, real_vessel_mask).to(self.device) * 0.00002 # output topology / fixed to 0.00001
            ...
            loss += calculate_topo_latent_loss(latent_fake, latent_real)*0.0001# * output_topology  # latent topology

        # ----------------- Vessel Structure Consistency Loss --------------
        # structural loss for vessel masks
        # loss += 1 - (2. * (pred_vessel_mask * real_vessel_mask).sum() + 1e-6) / (pred_vessel_mask.sum() + real_vessel_mask.sum() + 1e-6)

        # structural loss for vessel features
        ## get the vessel mask features
        with torch.no_grad(): real_vessel_features = unet.encoder(real_img.detach())[-1]
        pred_vessel_features = unet.encoder(pred_img)[-1]
        loss += F.l1_loss(pred_vessel_features, real_vessel_features) * 0.5

        # ### NEXT: use all features from encoder and loop over each feature map!!!! 
        # with torch.no_grad(): real_vessel_features = unet.encoder(real_img.detach())
        # pred_vessel_features = unet.encoder(pred_img)
        # for i in range(len(real_vessel_features)):
        #     loss += F.l1_loss(pred_vessel_features[i], real_vessel_features[i]) * 0.5
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            sample_condition_img1 = sample['cond_od_image'].unsqueeze(0).to(self.device)
            sample_condition_img2 = sample['cond_peripheral_image'].unsqueeze(0).to(self.device)
            # sample_cond = sample['fov_label'].unsqueeze(0).to(self.device) # class label
            sample_cond = sample['base_text'] # text label

            # sample_cond = condition[0:self.num_samples] if condition is not None else None
            sample_img = self.sample(num_samples=self.num_samples, img_size=x_0.shape[1:], steps=50, cond_img1=sample_condition_img1, cond_img2=sample_condition_img2, condition=sample_cond).detach()
            ...
            images_final = torch.cat((sample_condition_img1*255, sample_condition_img2*255, sample_target*255, images*255),0)
            save_image(images_final, path_out/f'sample_{log_step}.png', normalize=True)
        
        # torch.cuda.empty_cache() 
        return loss

    def forward(self, x_t, t, cond_img=None, cond_img2=None, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False, un_cond=None):
        ...
        if (condition is not None) and (guidance_scale != 1.0):
            # Model prediction 
            if self.image_conditioning:
                pred_uncond, _, _ = noise_estimator(x_t, t, cond_img=None, cond_img2=None, condition=un_cond, self_cond=self_cond)
                pred_cond, _, _ = noise_estimator(x_t, t, cond_img=cond_img, cond_img2=cond_img2, condition=condition, self_cond=self_cond)
            ...
            pred, _, _ =  noise_estimator(x_t, t, cond_img=cond_img, cond_img2=cond_img2, condition=condition, self_cond=self_cond)
            # pred, _ =  noise_estimator(x_t, t, cond_img=cond_img, condition=condition, self_cond=self_cond)
            if self.estimate_variance:
    ...
    @torch.no_grad()
    def denoise(self, x_t, steps=None, cond_img=None, cond_img2=None, condition=None, use_ddim=True, **kwargs):
        self_cond = None 
        ...
        for i, t in enumerate(reversed(timesteps_array)):
            ...
            # UNet prediction 
            x_t, x_0, x_T, self_cond = self(x_t, t.expand(x_t.shape[0]), cond_img, cond_img2, condition, self_cond=self_cond, **kwargs)
    ...
    @torch.no_grad()
    def sample(self, num_samples, img_size, cond_img1=None, cond_img2=None, condition=None, **kwargs):
        template = torch.zeros((num_samples, *img_size), device=self.device)
        x_T = self.noise_scheduler.x_final(template)
        x_0 = self.denoise(x_T, cond_img=cond_img1, cond_img2=cond_img2, condition=condition, **kwargs)
        return x_0 
