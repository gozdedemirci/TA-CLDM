...
import sys
sys.path.append("../../..")
from src.models.utils.conv_blocks import BasicBlock, UnetBasicBlock, UnetResBlock, save_add, BasicDown, BasicUp, SequentialEmb
from src.models.embedders.time_embedder import TimeEmbbeding
from src.models.utils.attention_blocks import Attention, zero_module


class UNet2PoseEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.se_projector = nn.Sequential(  
            nn.Linear(262144, 512), # 256 -- 1024*8*8, 512 -- 262144
            nn.ReLU(),
            nn.Linear(512, 64),
            nn.ReLU(),
            nn.Linear(64, 6),     # Affine parameters (2x3 matrix
            # nn.Tanh() # for V0 and V1 losses we don't use tanh()
        )  
        nn.init.zeros_(self.se_projector[4].weight)
        # nn.init.zeros_(self.se_projector[4].bias)
        self.se_projector[4].bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))

        self.se_embedder = nn.Sequential(
            nn.Linear(6, 512),
            nn.ReLU(),
            nn.Linear(512, 1024),
        ) 

    # def forward(self, x_t, t=None, cond_img=None, condition=None, self_cond=None): # this is for single condition input
    def forward(self, x_t, t=None, cond_img=None, cond_img2=None, condition=None, self_cond=None):
        # x_t [B, C, *]
        # t [B,]
        # condition [B,]
        # self_cond [B, C, *]
        # cond_img [B, C, *]
        # cond_vessel_mask [B, 1, *]
        ...
        if (condition is None) or (self.cond_embedder is None):
            ...
            # cond_emb = self.cond_embedder(cond_img, condition) # [B, C]
            cond_emb = self.cond_embedder(cond_img, cond_img2, condition) # [B, C]
        
        emb = save_add(time_emb, cond_emb)
       
        # ---------- Self-conditioning-----------
        if self.use_self_conditioning:
            self_cond =  torch.zeros_like(x_t) if self_cond is None else self_cond 
        ...
        temp = h.flatten(1) # [10, 1024, 8, 8]
        se2_prediction = self.se_projector(temp)
        se2_embedding = self.se_embedder(se2_prediction)

        emb_final = save_add(emb, se2_embedding) # after bottleneck, the condition will include the SE(2) embedding
        ...
        for i in range(len(self.out_blocks), 0, -1):
            ...
            h = self.out_blocks[i-1](h, emb_final)

    
        # ---------Out-Convolution ------------
        y = self.outc(h)

        return y, y_ver[::-1],se2_prediction




if __name__=='__main__':
    model = UNet2PoseEncoder(in_ch=3, use_res_block=False, learnable_interpolation=False, use_self_conditioning=True)
    input = torch.randn((1,3,16,32,32))
    input_cond = torch.randn((1,3,16,32,32))
    class_cond = torch.tensor([3,])
    time = torch.randn((1,))
    out_hor, out_ver, se2 = model(input, time, class_cond)
