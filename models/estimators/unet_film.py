...
from src.models.utils.film_module import FiLM
...
class UNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.film_down = FiLM(feature_dim=1024, cond_dim=1024)
        ...
        self.film_bottleneck = FiLM(feature_dim=1024, cond_dim=1024)
        ...
        self.film_up = FiLM(feature_dim=256, cond_dim=1024)
    ...
    def forward(self, x_t, t=None, cond_img=None, condition=None, self_cond=None):
        ...
        # ---------- FiLM-Middle --------------
        x_temp = self.film_down(x[-1], cond_emb if cond_emb is not None else emb)
        x[-1] = x_temp

        # ---------- Middle --------------
        h = self.middle_block(x[-1], emb)
        h = self.film_bottleneck(h, cond_emb if cond_emb is not None else emb)
        ...
        h = self.film_up(h, cond_emb if cond_emb is not None else emb)
        # ---------Out-Convolution ------------
        y = self.outc(h)
...
if __name__=='__main__':
    from src.models.embedders.time_embedder import TimeEmbbeding
    from src.models.embedders.cond_embedders import JointConditionEncoder

    cond_embedder = JointConditionEncoder 
    cond_embedder_kwargs = {
        'embed_dim': 1024,
        'label_dim': 2
    }
    ...
    input_cond = torch.randn((1,3,256, 256))
    class_cond = torch.tensor([1,])
    time = torch.randn((1,))
    out_hor, out_ver = model(input, time, input_cond, class_cond)
