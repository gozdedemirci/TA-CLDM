...
"""
This is a code for output level topology consistency loss in our diffusion model. We apply this loss into our diffusion model as follows:

- We use DDPM to generate RGB image via latent diffusion model
- Generate vessel mask of this generated RGB fundus image (this will be our likelihood)
- Generate vessel mask of the real RGB fundus image (this will be our gt)
- use "calculate_topo_loss" fn to calculate loss function.
PS: please be aware of the order of inputs, the loss function should have gradient to use as backwards.

Credits for topology loss: https://github.com/Melon-Xu/TopoSemiSeg/
...
def getTopoLoss(stu_tensor, tea_tensor, topo_size=100, pd_threshold=0.7, loss_mode="mse"):
    ...
    topo_cp_weight_map = torch.tensor(topo_cp_weight_map, dtype=torch.float).to(stu_tensor.device)
    topo_cp_ref_map = torch.tensor(topo_cp_ref_map, dtype=torch.float).to(stu_tensor.device)
...
if __name__ == "__main__":
    ...
    likelihood = torch.rand(1, 1, 256,256).cuda()
    likelihood.requires_grad = True
    gt = torch.rand(1, 1, 256,256).cuda()
    gt.requires_grad = False
    loss = calculate_topo_loss(likelihood, gt)
