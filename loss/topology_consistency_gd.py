...
def getTopoLoss(stu_tensor, tea_tensor, topo_size=100, pd_threshold=0.7, loss_mode="mse"):
    ...
    if topo_size > likelihood.shape[0]:
                    ...
                    if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[
                        ...
                        if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                            ...
                            topo_cp_ref_map[y + int(bcp_lh[hole_indx][0]), x + int(bcp_lh[hole_indx][1])] = \
                                gt_patch[int(dcp_lh[hole_indx][0]), int(dcp_lh[hole_indx][1])]
                    ...
                    if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                        ...
                        if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[
                            ...
                            topo_cp_ref_map[y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = \
                                gt_patch[int(bcp_lh[hole_indx][0]), int(bcp_lh[hole_indx][1])]
                                # gt_patch[int(bcp_lh[hole_indx][0]), int(bcp_lh[hole_indx][1])]
                        else:
                            topo_cp_ref_map[y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = 0
                            # topo_cp_ref_map[y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = 0

    cv2.imwrite("topo_cp_weight_map.png", topo_cp_weight_map * 255)
    cv2.imwrite("topo_cp_ref_map.png", topo_cp_ref_map * 255)
...
if __name__ == "__main__":
    # Test the function
    # make likelihoos with grad
    import time
    import cv2

    vessel1 = cv2.imread("/home/gdemi/multi_view/temp/training/vessel_fake.png", cv2.IMREAD_GRAYSCALE)
    vessel2 = cv2.imread("/home/gdemi/multi_view/temp/training/vessel_real.png", cv2.IMREAD_GRAYSCALE)
    vessel3 = cv2.imread("/home/gdemi/multi_view/temp/training/vessel.png", cv2.IMREAD_GRAYSCALE)

    vessel1 = cv2.resize(vessel1, (256, 256))
    vessel2 = cv2.resize(vessel2, (256, 256))
    vessel3 = cv2.resize(vessel3, (256, 256))

    vessel1 = torch.tensor(vessel1, dtype=torch.float).unsqueeze(0).unsqueeze(0)#.cuda()
    vessel2 = torch.tensor(vessel2, dtype=torch.float).unsqueeze(0).unsqueeze(0)#.cuda()
    vessel3 = torch.tensor(vessel3, dtype=torch.float).unsqueeze(0).unsqueeze(0)#.cuda()

    vessel_fake = vessel1 / 255.0
    vessel_gt = vessel2 / 255.0
    vessel_cond = vessel3 / 255.0

    vessel_fake.requires_grad = True

    start_time = time.time()

    vessel_x = vessel_gt.clone()
    vessel_x.requires_grad = True
    # loss = calculate_topo_loss(vessel_fake, vessel_cond) # expect bigger topology loss
    loss = calculate_topo_loss(vessel_fake, vessel_gt) # expect smaller topology loss

    print(loss)
    # loss_true = calculate_topo_loss(vessel_fake, vessel_gt)
