    ...
    def __init__(self, input_nc=4, ndf=64, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        ...
        def discriminator_block(in_filters):
            block = [
                ...
                nn.Conv2d(in_filters, in_filters * 2, kernel_size=4, stride=2, padding=1),
                norm_layer(in_filters * 2),
            ]
            return block
...
if __name__ == "__main__":
    ...
    # model = GeneratorUNet(in_channels=3, out_channels=1)
    # output = model(input)
    output = torch.randn(1,1,400,400)
    print(output.shape)
    disc = PixelDiscriminator(input_nc=4, ndf=64, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[])
    output = disc(input, output)
    print(output.shape)